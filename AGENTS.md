...
- Config lives at `$XDG_CONFIG_HOME/homelab-dns/config.yaml`; env override prefix is `HLDNS_` (dots → underscores). Token is set by `hl dns login` and persisted via `config.SetToken`.
